...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const response = await post<{ message: string }>("https://127.0.0.1:8000/api/login", { email, password });
      ...
      <div className="w-full max-w-130 bg-neutral-50 p-10 rounded-md flex flex-col items-center gap-3">
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 relative">
            ...
            <input type={showPassword ? "text" : "password"} name="password" id="password" value={password} onChange={(e: ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)} className="shadow-sm border border-neutral-300 rounded-md p-3 pr-10 text-lg" />
