...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const response = await post<{ message: string }>("https://127.0.0.1:8000/api/register", { pseudo, email, plainPassword });
  ...
  return (
    <main className="w-full min-h-dvh flex justify-center items-center">
      <div className="w-full max-w-130 bg-neutral-50 p-10 rounded-md flex flex-col items-center gap-3">
        <h1 className="font-bold text-3xl">Créer un compte</h1>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
          <TextInput label="Pseudo" id="pseudo" value={pseudo} onChange={(e) => setPseudo(e.target.value)} />
          ...
          <div className="flex flex-col gap-3 relative">
            ...
            <input type={showPassword ? "text" : "password"} name="password" id="password" value={plainPassword} onChange={(e) => setPlainPassword(e.target.value)} className="shadow-sm border border-neutral-300 rounded-md p-3 pr-10 text-lg" />
